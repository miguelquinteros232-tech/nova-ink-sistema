...
# Inicializamos el autenticador con la estructura correcta
...
if not st.session_state.get("authentication_status"):
    ...
    with tab_login:
        # Esto genera los campos de usuario y contraseña
        authenticator.login(location='main')
    
    with tab_reg:
        try:
            # Quitamos 'pre_authorization' que causaba el error
            if authenticator.register_user(location='main'):
                with open("config_pro.yaml", 'w') as f:
                    yaml.dump(config, f, default_flow_style=False)
                st.success('¡Registro exitoso! Ya puedes ir a la pestaña de Acceso.')
        except Exception as e:
            st.error(f"Error al registrar: {e}")

# --- 4. APLICACIÓN PRINCIPAL (Si el acceso es correcto) ---
    ...
    if menu == "📊 DASHBOARD":
        ...
        if not df_p.empty:
            ...
            for i, r in df_p.iterrows():
                bloque = r['Estado'] == "Vendido"
                with st.expander(f"{'🔒' if bloque else '⚙️'} {r['ID']} - {r['Cliente']}"):
                    if bloque: st.info("Pedido Finalizado.")
                    else:

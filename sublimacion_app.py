...
if not st.session_state.get("authentication_status"):
    ...
    auth_t1, auth_t2 = st.tabs(["🔐 ACCESO", "📝 REGISTRO"])
    with auth_t1: authenticator.login(location='main')
    with auth_t2:
        if authenticator.register_user(location='main'):
            with open("config_pro.yaml", 'w') as f: yaml.dump(config, f)
            st.success('Registrado')
else:
    # --- 3. CONEXIÓN (FORZANDO LA URL DESDE SECRETS) ---
    # Esto evita el error SpreadsheetNotFound
            ...
            for i, r in df_p.iterrows():
                ...
                with st.expander(f"{'🔒' if bloqueado else '⚙️'} {r['ID']} - {r['Cliente']}"):
                    if bloqueado:
                        ...
                        st.table(pd.DataFrame([r[['Fecha', 'Producto', 'Monto', 'Gasto_Prod']]]))
                    else:
                        with st.form(f"f_{i}"):
                            ...
                            new_des = st.text_area("Notas", value=r.get('Descripcion', ''))
                            if st.form_submit_button("Sincronizar"):
                                df_p.at[i, 'Estado'], df_p.at[i, 'Monto'], df_p.at[i, 'Descripcion'] = new_est, new_mon, new_des
                                conn.update(spreadsheet=URL_HOJA, worksheet="Pedidos", data=df_p)
                                st.rerun()

    # --- B. STOCK (ESTRUCTURA COMPLETA) ---
    elif menu == "📦 STOCK":
        df_inv = conn.read(spreadsheet=URL_HOJA, worksheet="Inventario", ttl=0)
        with st.form("add_stock"):
            ...
            col, can = c1.text_input("Color"), c2.number_input("Cantidad", min_value=0)
    ...
    # --- C. NUEVO PEDIDO (DESCUENTO AUTOMÁTICO) ---

...
if st.session_state.get("authentication_status") is not True:
    ...
    # FORZAMOS LOS CAMPOS DE REGISTRO
        ...
        with st.form("registro_manual"):
            ...
            if st.form_submit_button("REGISTRAR USUARIO"):
                if new_email and new_username and new_password:
                    try:
                        # Encriptamos y guardamos manualmente en el config
                        ...
                        st.success("✅ ¡Usuario creado! Ahora puedes subir e iniciar sesión.")
                        time.sleep(2)
                        st.rerun()
...
# --- 4. APLICACIÓN PRINCIPAL (Solo si está autenticado) ---
    ...
    if sh:
        ...
        if menu == "📊 DASHBOARD":
            ...
            tab1, tab2 = st.tabs(["NUEVO", "MODIFICAR"])
            ...
            with tab1:
                with st.form("n_p"):
                    ...
                    if st.form_submit_button("Guardar"):
                        ...
                        ws_p.append_row([len(ws_p.get_all_values()), datetime.now().strftime("%d/%m/%Y"), cli, prd, det, mon, "Producción", 0, "No Pago"])
                        st.success("Registrado."); st.rerun()
            with tab2:
                df_e = pd.DataFrame(ws_p.get_all_records())
                if not df_e.empty:
                    sel = st.selectbox("Pedido", df_e['Cliente'] + " | " + df_e['Producto'])
                    idx_e = df_e[df_e['Cliente'] + " | " + df_e['Producto'] == sel].index[0]
                    with st.form("e_o"):
                        u_det = st.text_area("Detalle", value=df_e.iloc

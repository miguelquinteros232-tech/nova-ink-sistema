...
# Importación segura de excepciones
try:
    from gspread.exceptions import PermissionDenied, WorksheetNotFound
except ImportError:
    # Fallback por si la versión de gspread es antigua
...
if not st.session_state.get("authentication_status"):
    ...
    if menu == "📊 DASHBOARD":
        ...
        if not df_p.empty:
            ...
            v_netas = df_p[df_p['Estado'] == 'Vendido']['Monto'].sum()
            g_total = df_p['Gasto_Prod'].sum()
            
            c1, c2, c3 = st.columns(3)
            c1.metric("INGRESOS", f"${v_netas:,.2f}")
            c2.metric("COSTOS", f"${g_total:,.2f}")
            c3.metric("UTILIDAD", f"${v_netas - g_total:,.2f}")

            for i, r in df_p.iterrows():
                bloque = r['Estado'] == "Vendido"
                with st.expander(f"{'🔒' if bloque else '⚙️'} {r['ID']} - {r['Cliente']}"):
                    if bloque: st.info("Finalizado.")
                    else:
